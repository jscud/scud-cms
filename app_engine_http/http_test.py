...
class ClientTest(unittest.TestCase):
  ...
  def test_get_google_dot_com(self):
    client = http.Client()
  ...
  def test_client_login(self):
    ...
    self.assertEqual(resp.status, '200')
    self.assertEqual(resp.reason, 'OK')
    self.assertFalse(resp.body.startswith(
        'I\'m sorry, I don\'t know who you are...'))
    self.assertTrue(resp.body.startswith('Hello, '))


class UriTest(unittest.TestCase):
  
  def test_parse_uri(self):
    uri = http.parse_uri('http://www.google.com/test?q=foo&z=bar')
  ...
  def test_modify_request_http_with_set_port(self):
    request = http.HttpRequest(uri=http.Uri(port=8080),
                                         method='POST')
    request.add_body_part('hello', 'text/plain') 
    uri = http.parse_uri('//example.com/greet')
  ...
  def test_modify_request_use_default_ssl_port(self):
    request = http.HttpRequest(
        uri=http.Uri(scheme='https'), method='PUT')
    request.add_body_part('hello', 'text/plain')
    uri = http.parse_uri('/greet')
  ...
  def test_to_string(self):
    uri = http.Uri(host='www.google.com', query={'q':'sippycode'})
...
class HttpRequestTest(unittest.TestCase):
  ...
  def test_copy(self):
    request = http.HttpRequest(
        uri=http.Uri(scheme='https', host='www.google.com'),
